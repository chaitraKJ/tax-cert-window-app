...
 * ═══════════════════════════════════════════════════════════════════════ */
const CONFIG = {
  BASE_URL: "https://taxes.paystar.io/app/customer/jasper-county-tax/search/property-taxes",
...
  const structuredLog = { level: level.toUpperCase(), timestamp, service: "jasper-scraper", parcel, step, message, ...meta };
...
  if (!parcel || typeof parcel !== "string") throw new ValidationError("Parcel must be a non-empty string", "parcel");
  const trimmed = parcel.trim();
  if (trimmed.length === 0) throw new ValidationError("Parcel cannot be empty", "parcel");
  // Allow any alphanumeric and hyphens/spaces for input validation
  const sanitized = trimmed.replace(/\s+/g, "-");
  if (sanitized.length < 5) throw new ValidationError("Parcel must be at least 5 chars", "parcel");
  if (!/^[a-zA-Z0-9\-]+$/.test(sanitized)) throw new ValidationError("Parcel contains invalid chars", "parcel");
...
  const due = new Date(y + 1, CONFIG.TAX_DATES.DUE_MONTH, CONFIG.TAX_DATES.DUE_DAY);
  const delq = new Date(y + 1, CONFIG.TAX_DATES.DELQ_MONTH, CONFIG.TAX_DATES.DELQ_DAY);
  return { dueDate: formatDate(due), delqDate: formatDate(delq) };
};

const isYearDelinquent = (year, now = new Date()) => {
  const delq = new Date(parseInt(year, 10) + 1, CONFIG.TAX_DATES.DELQ_MONTH, CONFIG.TAX_DATES.DELQ_DAY);
  return now >= delq;
};
...
  } catch (e) { throw new SelectorError(sel, parcel, step); }
};
...
    if (val !== text) log("warn", parcel, step, `Value mismatch: expected "${text}", got "${val}"`);
  } catch (e) { throw new SelectorError(sel, parcel, step); }
};
...
        await typeClear(page, CONFIG.SELECTORS.SEARCH_INPUT, parcel, parcel, step);
        
        // 3. Re-submit the search and wait for the results table to appear
        await Promise.all([
...
        ]);
        
        await page.waitForSelector(CONFIG.SELECTORS.TABLE, { visible: true, timeout: CONFIG.TIMEOUTS.SELECTOR });
    } catch (e) {
        // If the re-search fails (e.g., input or submit button not found)
        throw new NavigationError("Failed to rerun search and return to search results list", parcel);
    }
};
...
      for (let dl of dls) {
        const dt = Array.from(dl.querySelectorAll("dt.key")).find((el) =>
...
          if (!dd) continue;
          
          // **REFINED LOGIC:** Handling the nested anchor tag and cleaning the parcel number
          if (key === "Parcel Number") {
            const a = dd.querySelector("a");
            let val = (a ? a.textContent : dd.textContent);
            // Crucial: Use strict regex to remove all non-alphanumeric chars
...
      assessed_value: getItemValue("Assessed Value").replace(/[^\d.-]/g, "") || "0",
      taxable_appraised_value: getItemValue("Taxable Appraised Value").replace(/[^\d.-]/g, "") || "0",
...
  const calculated = calculateTaxDates(row.year);

  // 2. Handle case where detail scrape (and verification) failed completely.
  if (!details) {
    return {
      jurisdiction: "County",
...
      amount_due: "$0.00",
      paid_date: "N/A",
      // ACTION: Use hardcoded dates when details are null
...
  const costs = parseFloat(details.costs) || 0;
  
  // 3. Determine Final Dates:
  // ACTION: ONLY use the hardcoded calculated dates
  const dueDate = calculated.dueDate; 
  const delqDate = calculated.delqDate;
  
  const paidDate = details.payment_date !== "N/A" ? formatScrapedDate(details.payment_date) : "";
...
    return { notes: `ALL PRIORS ARE PAID,${latestYear} TAXES ARE ${latestStatus}, ${NOTE}`, delinquent: "NONE" }; // This case should handle when no unpaid records exist
  }
  
...
    return { notes: `ALL PRIORS ARE PAID, ${latestYear} TAXES ARE ${latestStatus}, ${NOTE}`, delinquent: "NONE" };
  }
  
  const latestYear = latestRecord.year;
  const latestStatus = latestRecord.status;
  
  // Check if any prior year (before the latest year) has a DELINQUENT status
  const hasDelinquentPrior = taxHistory.some((r) => r.status === "DELINQUENT" && parseInt(r.year) < parseInt(latestYear));
...
    await typeClear(page, CONFIG.SELECTORS.SEARCH_INPUT, parcel, parcel, "SEARCH_INPUT");
    await Promise.all([
      page.click(CONFIG.SELECTORS.SUBMIT_BTN),
      page.waitForNavigation({ waitUntil: "networkidle0", timeout: CONFIG.TIMEOUTS.NAVIGATION }).catch(() => {}),
    ]);
    await page.waitForSelector(CONFIG.SELECTORS.TABLE, { timeout: CONFIG.TIMEOUTS.SELECTOR });

    // 2. Extract all rows from table
    const historyRaw = await page.evaluate((tableSel, maxYears) => {
...
        const status_text = status_span ? status_span.textContent.trim() : "";
...
       .filter((r, i, arr) => r.year >= arr[arr.length - 1].year - maxYears + 1); 
    }, CONFIG.SELECTORS.TABLE, CONFIG.MAX_YEARS_BACK);

    if (historyRaw.length === 0) throw new NoRecordsError(parcel);

    const ownerFromTable = historyRaw[historyRaw.length - 1].owner.replace(/&amp;/g, "&");

    // 3. Identify records to scrape and records to return.
    const latestRow = historyRaw[historyRaw.length - 1]; 
    const unpaidRows = historyRaw.filter(r => r.status_text.toLowerCase() !== "paid");
      
    let rowsToScrape = []; 
...
    
    // 1. Add the latest bill (highest year) - ensures property details are scraped.
    if (latestRow) {
...
    rowsToScrape = Array.from(scrapeMap.values());
    
    // Process List (Final output):
...
        log("info", parcel, "FILTER", "All records are paid. Returning only the latest paid year.");
    } else {
...
    rowsToScrape.sort((a, b) => b.year - a.year); 

    // 4. Scrape detail pages with verification
...
        await page.goto(`${CONFIG.BASE_DOMAIN}${row.view_href}`, { waitUntil: "networkidle0", timeout: CONFIG.TIMEOUTS.NAVIGATION });
      }, { parcel, step: `DETAIL_GOTO_${row.year}` });
      await waitForPageReady(page);
      await page.waitForSelector(CONFIG.SELECTORS.DETAIL_DL, { timeout: CONFIG.TIMEOUTS.SELECTOR });
      
...
      if (isZeroRecord) {
        log("warn", parcel, `ZERO_RECORD_${row.year}`, `Ignoring record for year ${row.year} as all key financial fields are zero.`);
        // Remove this receipt from the details map if it somehow got added earlier
        detailsMap.delete(row.receipt); 
        
        // If the record was in the original `recordsToProcess` list, we must remove it so it's not processed later.
        recordsToProcess = recordsToProcess.filter(r => r.receipt !== row.receipt);
        
...
        log("warn", parcel, `VERIFY_${row.year}`, `Parcel mismatch: Expected ${expectedBaseParcel}, found ${scrapedParcel}. Skipping record.`);
        
        if (i === 0) {
          log("error", parcel, `VERIFY_${row.year}`, "Mismatch on LATEST year/receipt. Cannot guarantee property data integrity.");
        }
        
...
      detailsMap.set(row.receipt, details);

      // Extract property info (only done once, from the first scraped record, which should be the latest)
      if (i === 0) {
...
    recordsToProcess.sort((a, b) => a.year - b.year);

    // 6. Build final tax history records
...
    taxHistory = taxHistory.filter(record => record._verified === true);
    
...
    page.on("request", (r) => {
      if (["stylesheet", "font", "image", "media"].includes(r.resourceType())) r.abort();
      else r.continue();
    });
    const data = await withRetry(() => getTaxData(page, parcel), { parcel, step: "CORE_FETCH" });
    if (fetch_type === "html") {
...
    const payload = isNo ? handleNotFound(e.context.parcel) : { error: true, message: e.message || "Error", code: e.code || "UNKNOWN" };
