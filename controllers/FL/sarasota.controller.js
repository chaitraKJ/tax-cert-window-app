...
const ac_1 = async (page, url, account) => {
	return new Promise(async (resolve, reject) => {
		try{
			...
			// CLICK ENTER AND WAIT FOR URL CHANGE
			await page.waitForSelector("#TabSearch-panel button", timeout_option);
			Promise.all([
				page.locator("#TabSearch-panel button").click(),
				page.waitForNavigation()
			])
			.then(() => {

				page.waitForSelector("#TabSearch-panel h4", timeout_option)
				.then(async () => {

					const parcel_search_data = await page.evaluate(() => {
						const account_ele = document.querySelector("#TabSearch-panel h4");
						const text = account_ele?.textContent.trim();
						if(text.includes("Account")){
							const parent_next = account_ele?.parentElement?.nextElementSibling;

							const owner = account_ele?.nextElementSibling?.textContent.trim();
							const link = parent_next?.querySelector("a").href;							
							const address = parent_next?.querySelector("strong")?.textContent.trim();

							return { link, owner, address };
						}
						else if(text.includes("No Result Found")){
							return null;
						}
					});

					if(!parcel_search_data){
						reject(new Error("Account Not Found"));
					}
					else{
						resolve(parcel_search_data);
					}
				})
				.catch((error) => {
					console.log(error);
...
const ac_2 = async (page, data) => {
	return new Promise(async(resolve, reject) => {
		try{
			const status = await page.goto(data['link'], { waitUntil: "domcontentloaded"});
...
const ac_3 = async (main_data, account) => {
	return new Promise(async(resolve, reject) => {
		try{
			let th = {
				jurisdiction: "County",
			...
			// MANIPULATE THE DATA
			let { owner, address } = main_data['data'];
			const data = {
				processed_date : "",
				order_number : "",
				borrower_name: "",
