...
const levy_1 = async (page, account) => {
  return new Promise((resolve, reject) => {
    (async () => {
      try {
        const url = "https://levyitm.wfbsusa.com/PropertySearchAccount.aspx";
...
/* -------------------- Step 2 -------------------- */
const levy_2 = async (page, account, yearsRequested = 1) => {
  return new Promise((resolve, reject) => {
    (async () => {
      try {
        ...
        const page_data = await page.evaluate((account, yearsRequested = 1) => {
              ...
              if (
                ...
                const fallbackValue =
                  ...
                  Math.round(
                    ...
                    ) / 0.0185 // Levy County approximate tax rate
                  ).toLocaleString();
          ...
          if (tblSummary) {
            ...
            if (hasInstallment && tblInstallSched) {
              // Extract data from installment table (following Leon County approach)
              ...
              installRows.forEach((row, index) => {
                if (index > 1) { // Skip header rows
                  ...
                  if (cells.length >= 4) {
                    ...
                    if (paidStatus === "Unpaid" && delq_date) {
                      const delqDateObj = new Date(delq_date);
                      if (delqDateObj < currentDate) {
                    ...
                    tax_history.push({
                      ...
                      status: paidStatus === "Unpaid" ? "Unpaid" : "Paid",
                      base_amount: base_amount,
                      amount_paid: paidStatus === "Paid" ? "$" + baseAmount : "$0.00",
                      amount_due: paidStatus === "Unpaid" ? "$" + baseAmount : "$0.00",
                      mailing_date: "N/A",
                      due_date: due_date,
                      delq_date: delq_date,
                      paid_date: paidStatus === "Paid" ? discountDate : "-",
                      good_through_date: "",
                    });
                  }
                }
              ...
              // Extract from tax history table if no installment (same logic as Leon)
              let latestYear = 0;
              const currentDate = new Date("2025-09-15");
              summaryRows.forEach((row) => {
                const cells = row.querySelectorAll("td");
                if (cells.length >= 7) {
                  ...
                  if (
                    ...
                    tax_history.push({
                      ...
                      amount_paid:
                        "$" + (processedStatus === "Due" || processedStatus === "Unpaid" || processedStatus === "Delinquent" || processedStatus === "MINTAX" || processedStatus === "No Tax Due" ? "0.00" : amountPaid),
                      amount_due:
                        "$" + (processedStatus === "Due" || processedStatus === "Unpaid" || processedStatus === "Delinquent" || processedStatus === "MINTAX" || processedStatus === "No Tax Due" ? amountDue : "0.00"),
                      mailing_date: "N/A",
            ...
            console.log(
              "Table #ctl00_ContentPlaceHolder1_tblSummary not found"
            );
          }
          ...
          const isQuarterly = tax_history.some((row) =>
            row.payment_type.includes("installment")
          );
          const paymentFrequency = isQuarterly ? "INSTALLMENTS" : "ANNUALLY";
          const dueDates = isQuarterly
            ? "06/30, 10/30, 12/31 & 03/31"
            : "03/31";
          let notes = "";
          ...
          return {
            ...
            taxing_authority:
              "Levy County Tax Collector, 625 N. Pine Ave., Chiefland, FL 32626, Ph: 352-493-4726",
            notes,
